...
import fnmatch
try:
        from gadgetconfig.sysfstree.sysfstree import sysfstree
except (ImportError):
        ...
        def nb_update(self):
                ...
                if self.currentID == 1:
                       for s in os.listdir("/sys/class/udc"):
                        ...
                        #print('nb_update: self.m.realudcpath): %s' % (self.m.realudcpath), file=sys.stderr)
                        #s = sysfs(self.m.realudcpath, -1, sort=True, )
                        #s = sysfs('/sys/devices/platform/axi', -1, sort=True, )
                                #include=["*.usb", ["udc"], [], ["soft_connect", "function", "maximum_speed", "state", "uevent"]],
                                #bold=[["*"], ["UDC"]])
                        #print('nb_update: : %s' % (s), file=sys.stderr)


                elif self.currentID == 0:
                        s = sysfs(['/sys/kernel/config/usb_gadget/'], -1, 
